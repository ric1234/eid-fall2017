...
class App(QWidget):
	...
	def on_click_temperature(self):
		value=get_data('temperature')
		QMessageBox.information(self, 'Weather App', "The Temperature is: " + value+"Celsius\nTime is:"+strftime("%Y-%m-%d %H:%M:%S", gmtime()), QMessageBox.Ok, QMessageBox.Ok)
	def on_click_humidity(self):
		value=get_data('humidity')
	...
	def on_click_record(self):
		...
		for i in range (0,9):
			valueTemperature[i]=get_data('temperature')
			valueHumidity[i]=get_data('humidity')
	...
	def on_click_server(self):
		valueTemperature=get_data('temperature')
		valueHumidity=get_data('humidity')
